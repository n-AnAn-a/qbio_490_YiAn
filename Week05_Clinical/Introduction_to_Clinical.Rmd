...
```{r}
getwd()
setwd("/home1/yianxu/490_cluster/Week05_Clinical")
```
...
```{r}
write.csv(clinic, "/home1/yianxu/490_cluster/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/home1/yianxu/490_cluster/analysis_data/brca_clinical_data.csv")
```
...
```{r Exercise 4.1}
# create plot here
plot(clinic$age_at_diagnosis, clinic$days_to_last_followup,
     main="Age vs Days to Last Follow-up",
     xlab="Age at Diagnosis", ylab="Days to Last Follow-up")
```
...
boxplot(clinic$age_at_diagnosis ~ clinic$gender,
        main="Age at Diagnosis by Gender",
        xlab="Gender", ylab="Age at Diagnosis")
```
...
Time since diagnosis (measured in days).
# 3. What age group appears to have the greatest rate of survival?
Middle age group overtime which is almost 0.5.
# 4. What age group appears to have the worst rate of survival? 
Old age group initially but Young age group over time.
...
The p-value is 0.2. Since 0.2 > 0.05, this result is not statistically significant. Based on this dataset, there is no strong evidence that age group significantly affects breast cancer survival. Though the trends suggest differences, they are not statistically confirmed.
